...
class ValidationReport extends connect(store, LitElement) {
  ...
  _renderResultVisualisation() {
    switch (this.displayAs) {
      case 'tree':
        return html`<error-summary .validationResults="${this.results}"></error-summary>`
      case 'raw': {
        import('@rdfjs-elements/rdf-snippet')
        const raw = ntriples`${[...this.report.dataset]}`.toString()
        return html`
          <rdf-snippet .input="${raw}"
                       input-format="application/n-triples"
                       layout="vertical"
                       prefixes="sh"
                       formats="text/turtle,application/ld+json,application/rdf+xml"
                       only-output></rdf-snippet>`
      }
      default:
