...
function autoRefresh() {
  if (!autorefreshLoaded) {
    autorefreshLoaded = new Promise(resolve => {
      ...
      script.src = 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.32.0/addon/display/autorefresh.min.js'
...
class GraphEditor extends connect(store, LitElement) {
  ...
  render() {
    return html`
      <rdf-editor .format="${this.format}"
                  .serialized="${this.graph}"
                  auto-parse
                  .parseDelay="10"
                  .prefixes="${this.prefixes.join(',')}"
                  @quads-changed="${this.__quadsChanged}"></rdf-editor>`
  }
