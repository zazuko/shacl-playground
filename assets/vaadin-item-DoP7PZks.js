import{r as u,i as m,R as k,E as w,f as h,P as d,h as p,F as I,O as L,D as E,Q as A,C as O,n as P,m as V}from"./index-BfYWPPsp.js";import{a as q,r as F}from"./dom-utils-DPL7MVhW.js";u("vaadin-form-layout",m`
    :host {
 ...
 * license.
 */class y extends k(w(h(d))){static get template(){return p`
      <style>
        :host {
          display: block;
          max-width: 100%;
 ...
 * See https://vaadin.com/commercial-license-and-service-terms for the full
 * license.
 */console.warn('WARNING: Since Vaadin 23.2, "@vaadin/vaadin-form-layout" is deprecated. Use "@vaadin/form-layout" instead.');const M=m`
  :host {
    display: flex;
    align-items: center;
...
`;u("vaadin-item",M,{moduleId:"lumo-item"});/**
 * @license
 * Copyright (c) 2000 - 2024 Vaadin Ltd.
 *
 * This program is available under Vaadin Commercial License and Service Terms.
 ...
 * See https://vaadin.com/commercial-license-and-service-terms for the full
 * license.
 */class g extends L(h(E(d))){static get template(){return p`
      <style>
        :host {
    ...
    `}static get is(){return"vaadin-item"}constructor(){super(),this.value,this.label}ready(){super.ready(),this.setAttribute("role","option")}}customElements.define(g.is,g);u("vaadin-list-box",m`
    :host {
 ...
 * See https://vaadin.com/commercial-license-and-service-terms for the full
 * license.
 */const T=n=>class extends A(n){static get properties(){return{multiple:{type:Boolean,value:!1,reflectToAttribute:!0,observer:"_multipleChanged"},selectedValues:{type:Array,notify:!0,value:()=>[]}}}static get observers(){return["_enhanceMultipleItems(items, multiple, selected, disabled, selectedValues, selectedValues.*)"]}ready(){this.addEventListener("click",i=>this._onMultipleClick(i)),super.ready()}_enhanceMultipleItems(i,t,a,l,c){if(!(!i||!t)){if(c){const s=c.map(o=>i[o]);i.forEach(o=>{o.selected=s.includes(o)})}this._scrollToLastSelectedItem()}}_scrollToLastSelectedItem(){const i=this.selectedValues.slice(-1)[0];i&&!i.disabled&&this._scrollToItem(i)}_onMultipleClick(i){const t=this._filterItems(i.composedPath())[0],a=t&&!t.disabled?this.items.indexOf(t):-1;a<0||!this.multiple||(i.preventDefault(),this.selectedValues.includes(a)?this.selectedValues=this.selectedValues.filter(l=>l!==a):this.selectedValues=this.selectedValues.concat(a))}_multipleChanged(i,t){!i&&t&&(this.selectedValues=[],this.items.forEach(a=>{a.selected=!1}),this.removeAttribute("aria-multiselectable")),i&&!t&&(this.setAttribute("aria-multiselectable","true"),this.selected!==void 0&&(this.selectedValues=[...this.selectedValues,this.selected],this.selected=void 0))}};/**
 * @license
 * Copyright (c) 2000 - 2024 Vaadin Ltd.
 *
 * This program is available under Vaadin Commercial License and Service Terms.
 ...
 * See https://vaadin.com/commercial-license-and-service-terms for the full
 * license.
 */class x extends w(T(h(O(d)))){static get template(){return p`
      <style>
        :host {
          display: flex;
        }
 ...
 * See https://vaadin.com/commercial-license-and-service-terms for the full
 * license.
 */class N extends h(d){static get template(){return p`
      <style>
        :host {
          display: inline-flex;
          flex-direction: row;
    ...
    `}static get is(){return"vaadin-form-item"}constructor(){super(),this.__updateInvalidState=this.__updateInvalidState.bind(this),this.__fieldNodeObserver=new MutationObserver(()=>this.__updateRequiredState(this.__fieldNode.required)),this.__labelNode=null,this.__fieldNode=null}_getFieldAriaTarget(e){return e.ariaTarget||e}__linkLabelToField(e){q(this._getFieldAriaTarget(e),"aria-labelledby",this.__labelId)}__unlinkLabelFromField(e){F(this._getFieldAriaTarget(e),"aria-labelledby",this.__labelId)}__onLabelClick(){const e=this.__fieldNode;e&&(e.focus(),e.click())}__getValidateFunction(e){return e.validate||e.checkValidity}__onLabelSlotChange(){this.__labelNode&&(this.__labelNode=null,this.__fieldNode&&this.__unlinkLabelFromField(this.__fieldNode));const e=this.$.labelSlot.assignedElements()[0];e&&(this.__labelNode=e,this.__labelNode.id?this.__labelId=this.__labelNode.id:(this.__labelId=`label-${this.localName}-${V()}`,this.__labelNode.id=this.__labelId),this.__fieldNode&&this.__linkLabelToField(this.__fieldNode))}__onContentSlotChange(){this.__fieldNode&&(this.__unlinkLabelFromField(this.__fieldNode),this.__updateRequiredState(!1),this.__fieldNodeObserver.disconnect(),this.__fieldNode=null);const e=this.$.contentSlot.assignedElements();e.length>1&&console.warn(`WARNING: Since Vaadin 23, placing multiple fields directly to a <vaadin-form-item> is deprecated.
Please wrap fields with a <vaadin-custom-field> instead.`);const i=e.find(t=>!!this.__getValidateFunction(t));i&&(this.__fieldNode=i,this.__updateRequiredState(this.__fieldNode.required),this.__fieldNodeObserver.observe(this.__fieldNode,{attributes:!0,attributeFilter:["required"]}),this.__labelNode&&this.__linkLabelToField(this.__fieldNode))}__updateRequiredState(e){e?(this.setAttribute("required",""),this.__fieldNode.addEventListener("blur",this.__updateInvalidState),this.__fieldNode.addEventListener("change",this.__updateInvalidState)):(this.removeAttribute("invalid"),this.removeAttribute("required"),this.__fieldNode.removeEventListener("blur",this.__updateInvalidState),this.__fieldNode.removeEventListener("change",this.__updateInvalidState))}__updateInvalidState(){const e=this.__getValidateFunction(this.__fieldNode).call(this.__fieldNode);this.toggleAttribute("invalid",e===!1)}}customElements.define(N.is,N);/**
 * @license
 * Copyright (c) 2000 - 2024 Vaadin Ltd.
 *
 * This program is available under Vaadin Commercial License and Service Terms.
 ...
 * See https://vaadin.com/commercial-license-and-service-terms for the full
 * license.
 */console.warn('WARNING: Since Vaadin 23.2, "@vaadin/vaadin-item" is deprecated. Use "@vaadin/item" instead.');export{g as I,x as L};
